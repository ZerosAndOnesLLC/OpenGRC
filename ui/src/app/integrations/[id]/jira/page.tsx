...
export default function JiraDashboardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSync = async () => {
    try {
      ...
      toast({
        title: "Sync Started",
        description: "Jira data sync has been initiated.",
      })
      setTimeout(() => {
        refetchIntegration()
        refetchSyncLogs()
  ...
  const handleCollectEvidence = async () => {
    try {
      ...
      toast({
        ...
        description: `Created ${result.data.evidence_created} evidence records from Jira data.`,
      })
      refetchIntegration()
      refetchSyncLogs()
    } catch (error) {
  ...
  if (integrationLoading) {
    return <Loading message="Loading Jira dashboard..." />
  }
  ...
  if (!integration) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-muted-foreground mb-4">The requested Jira integration could not be found.</p>
        <Button asChild>
  ...
  if (integration.integration.integration_type !== 'jira') {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={accountInfo.instance_url ? `Instance: ${accountInfo.instance_url}` : 'Jira Cloud'}
        action={
      ...
      <div className="flex items-center gap-4">
        ...
        {accountInfo.instance_url && (
          <a
            href={accountInfo.instance_url}
            target="_blank"
        ...
        <TabsContent value="overview" className="mt-6">
          <div className="grid gap-6 md:grid-cols-2">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{accountInfo.display_name || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Email</p>
                    <p className="font-medium">{accountInfo.email || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Server Version</p>
                    <p className="font-medium">{accountInfo.server_version || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Deployment</p>
                    <p className="font-medium capitalize">{accountInfo.deployment_type || 'Cloud'}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  What Jira data is being collected for compliance evidence
                </CardDescription>
              </CardHeader>
          ...
          <Card className="mt-6">
            <CardHeader>
              ...
              <CardDescription>
                Compliance evidence automatically generated from Jira data
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div className="border rounded-lg p-4">
                  <h4 className="font-medium mb-1">Project Inventory</h4>
                  <p className="text-sm text-muted-foreground">Complete list of Jira projects and their settings</p>
                ...
                <div className="border rounded-lg p-4">
                  <h4 className="font-medium mb-1">Issues Overview</h4>
                  <p className="text-sm text-muted-foreground">Open issues by type, priority, and status</p>
                  <div className="mt-2 flex flex-wrap gap-1">
                ...
                <div className="border rounded-lg p-4">
                  <h4 className="font-medium mb-1">Security Issues Report</h4>
                  <p className="text-sm text-muted-foreground">Security-tagged issues and vulnerabilities</p>
                  <div className="mt-2 flex flex-wrap gap-1">
                    <Badge variant="outline" className="text-xs">CC7.1</Badge>
                ...
                <div className="border rounded-lg p-4">
                  <h4 className="font-medium mb-1">User Access Report</h4>
                  <p className="text-sm text-muted-foreground">Users and their project access</p>
                  <div className="mt-2 flex flex-wrap gap-1">
                    <Badge variant="outline" className="text-xs">CC6.1</Badge>
                ...
                <div className="border rounded-lg p-4">
                  <h4 className="font-medium mb-1">Project Roles Report</h4>
                  <p className="text-sm text-muted-foreground">Role assignments across projects</p>
                  <div className="mt-2 flex flex-wrap gap-1">
                    <Badge variant="outline" className="text-xs">CC6.1</Badge>
                    <Badge variant="outline" className="text-xs">CC6.2</Badge>
                    <Badge variant="outline" className="text-xs">CC6.3</Badge>
                ...
                <div className="border rounded-lg p-4">
                  <h4 className="font-medium mb-1">Admin Access Report</h4>
                  <p className="text-sm text-muted-foreground">Users with administrative privileges</p>
                  <div className="mt-2 flex flex-wrap gap-1">
                    <Badge variant="outline" className="text-xs">CC6.1</Badge>
                    <Badge variant="outline" className="text-xs">CC6.2</Badge>
                    <Badge variant="outline" className="text-xs">CC6.3</Badge>
        ...
        <TabsContent value="services" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Jira services being monitored for this integration
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div key={service} className="flex items-center justify-between py-2 border-b last:border-0">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {service === 'projects' && 'Syncs project metadata and settings'}
                          {service === 'issues' && 'Tracks open issues by type and priority'}
                          {service === 'users' && 'Monitors user access across projects'}
                          {service === 'permissions' && 'Tracks project role assignments'}
                        </p>
                      </div>
                      {enabled ? (
